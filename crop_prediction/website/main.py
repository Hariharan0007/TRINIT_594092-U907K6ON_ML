...
def map_state_to_soil(state):
    state_wise_soil = pd.read_csv(path + '\soil\\npk.csv')
...
def getPH(state):
    ph = pd.read_csv(path + '\water\\ph.csv', encoding='unicode_escape')
...
def getRainfall(district):
    ...
    rainfall_data = pd.read_csv(path + '\\rainfall\\district_rainfall.csv')
